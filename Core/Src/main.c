...
#define DDS_SIZE 1024
#define DAC_DMA_SAMPLES 2048
...
const osThreadAttr_t decodeTask_attributes = { .name = "decodeTask",
		.stack_size = 256 * 4, .priority = (osPriority_t) osPriorityNormal, };
/* Definitions for CAN_TX_TaskName */
osThreadId_t CAN_TX_TaskNameHandle;
const osThreadAttr_t CAN_TX_TaskName_attributes = { .name = "CAN_TX_TaskName",
		.stack_size = 128 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for HalfCpltCalcTas */
osThreadId_t HalfCpltCalcTasHandle;
const osThreadAttr_t HalfCpltCalcTas_attributes = { .name = "HalfCpltCalcTas",
		.stack_size = 1024 * 4, .priority = (osPriority_t) osPriorityRealtime, };
/* Definitions for FullCpltCalcTas */
osThreadId_t FullCpltCalcTasHandle;
const osThreadAttr_t FullCpltCalcTas_attributes = { .name = "FullCpltCalcTas",
		.stack_size = 1024 * 4, .priority = (osPriority_t) osPriorityRealtime, };
...
const osSemaphoreAttr_t CAN_TX_Semaphore_attributes = { .name =
		"CAN_TX_Semaphore" };
/* Definitions for HalfCplt */
osEventFlagsId_t HalfCpltHandle;
const osEventFlagsAttr_t HalfCplt_attributes = { .name = "HalfCplt" };
/* Definitions for FullCplt */
osEventFlagsId_t FullCpltHandle;
const osEventFlagsAttr_t FullCplt_attributes = { .name = "FullCplt" };
/* USER CODE BEGIN PV */
...
int16_t *lookup_tables[12] = { C_LUT, C_SHARP_LUT, D_LUT, D_SHARP_LUT, E_LUT,
		F_LUT, F_SHARP_LUT, G_LUT, G_SHARP_LUT, A_LUT, A_SHARP_LUT, B_LUT, };

uint16_t lookup_indices[12];

// DDS testing
volatile int16_t DDS_LUT[DDS_SIZE];
volatile uint16_t DDS_indices[12] = { 0 };
volatile uint16_t DDS_steps[12];
volatile uint16_t DDS_OUT[DAC_DMA_SAMPLES];
// end
...
const char *keyNotes[12] = { "Do", "Do#", "Re", "Re#", "Mi", "Fa", "Fa#", "Sol",
		"Sol#", "La", "La#", "Si" };
...
int16_t changeKnobState(uint8_t knob_state, uint8_t previousKnobState,
		uint16_t volume, int8_t top_limit, int8_t bottom_limit);
...
//		char stepText[10];
//		sprintf(stepText, "ha: %i", DDS_LUT[i]);
//		serialPrintln(stepText);
...
//		char stepText[10];
//		sprintf(stepText, "ah: %i", DDS_steps[i]);
//		serialPrintln(stepText);
...
inline void synthesize_waves(int index) {
	...
	for (int t = 0; t < 12; t++) {
		...
		if (pressed) {
			lookup_indices[t] = (lookup_indices[t] + (step_octave))
					% wavetable_sizes[t];
			keys_pressed += 1;
	...
	if (previousKnobState != knobState) {
		...
		if (type == 'v') {
			...
			int16_t change_octave = changeKnobState(knobState,
					previousKnobState, octave, 8, 2); // can only go one lower than the default octave
...
/* USER CODE BEGIN Header_HalfCpltCalc */
/**
...
void HalfCpltCalc(void *argument) {
	...
	/* USER CODE END HalfCpltCalc */
}

/* USER CODE BEGIN Header_FullCpltCalc */
/**
